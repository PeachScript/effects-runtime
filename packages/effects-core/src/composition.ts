...
export class Composition implements Disposable, LostHandler {
  ...
  constructor (
    ...
    this.speed = speed;
    this.autoRefTex = !this.keepResource && imageUsage && this.rootItem.endBehavior !== spec.EndBehavior.loop;
  ...
  private shouldRestart () {
    ...
    return ended && endBehavior === spec.EndBehavior.loop;
  }
  ...
  private toLocalTime (time: number) {
    ...
    if (localTime - duration > 0.001) {
      if (this.rootItem.endBehavior === spec.EndBehavior.loop) {
  ...
  private callUpdate (item: VFXItem, dt: number) {
    ...
    for (const child of item.children) {
      if (VFXItem.isComposition(child)) {
        if (
          child.ended &&
          child.endBehavior === spec.EndBehavior.loop
        ) {
          child.ended = false;
