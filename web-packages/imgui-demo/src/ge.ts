...
import { AnimationComponent, Component, FilterMode, GLSLVersion, Geometry, Material, OrderType, Player, PostProcessVolume, RenderPass, RenderPassPriorityPostprocess, RenderTextureFormat, RendererComponent, Texture, TransformAnimationPlayable, VFXItem, glContext, math } from '@galacean/effects';
...
import { DistortEffect } from './distort-effect';
...
export class GalaceanEffects {
  ...
  static async initialize () {
    ...
    GalaceanEffects.playURL('https://mdn.alipayobjects.com/mars/afts/file/A*DymuQLtOR80AAAAAAAAAAAAADlB4AQ');
  }
  ...
  static playURL (url: string, use3DConverter = false) {
    ...
    if (use3DConverter) {
      ...
      void GalaceanEffects.player.loadScene(url, { autoplay: true }).then(composition => {
        ...
        // const animationComponent = composition.getItemByName('sprite_1')?.addComponent(AnimationComponent);
        composition.getItemByName('null_90')?.addComponent(DistortEffect);
